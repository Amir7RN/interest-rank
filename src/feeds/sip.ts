...
/**
 * Per-second aggregates over the full SIP tape, from a Polygon-protocol socket.
 *
 ...
 *
 * Polygon and Massive speak the same wire protocol, so both are this class with
 * a different host — see `polygon.ts` and `massive.ts`.
 *
 * Requires a real-time plan with full-market aggregates. Delayed and IEX-only
 * tiers will distort a volume-based ranking badly.
 */
export class SipAggregateFeed implements Feed {
  readonly id: string;
  ...
  constructor(id: string, name: string, url: string, opts: FeedOptions) {
    this.id = id;
    this.name = name;
    this.url = url;
  ...
  start(onBars: (bars: Bar[]) => void, onStatus: (s: FeedStatus) => void): void {
    ...
    const connect = () => {
      ...
      onStatus({ state: 'connecting', detail: this.host });
